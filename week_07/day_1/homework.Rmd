...
ggplot(nyc_bikes_df) +
  geom_bar(aes(x = day, fill = gender), stat = "count", position = "dodge") +
  scale_x_continuous(breaks = 1:31) 

nyc_bikes_df

#if i can figure out what went wrong with the summarise function then the one below should be good

#nyc_bikes_df_summ_day %>%
 # ggplot() +
  #geom_point(aes(x = day, y = hires), group = gender, color = gender) + 
  #geom_line() +
  #scale_x_continuous(breaks = 1:31) 


#weekday comparison of gender

level_order_weekdays <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")

ggplot(nyc_bikes_df) +
  geom_bar(aes(x = factor(weekday, level = level_order_weekdays), fill = gender), stat = "count", position = "dodge") +
  scale_x_discrete() +
  xlab("\n Weekday") +
  ylab("Hires \n") +
  scale_fill_manual(values=c("#CC5500", "#800020", "#30D5C8")) +
